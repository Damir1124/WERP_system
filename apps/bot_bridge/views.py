    ...
    def get(self, request):
        ...
        for courier in all_couriers:
            ...
            if current_trip:
                ...
                # Вода надо доставить: сумма quantity для product с type_product='19W' (WATER) из взятых заказов
...
class CourierCreateOrderView(APIView):
    ...
    def post(self, request):
        ...
        # Проверяем активный рейс (опционально, заказ может быть создан без рейса)
        active_shift = CourierShift.objects.filter(
            courier=courier,
            status=CourierShift.Status.OPEN
        ).first()
