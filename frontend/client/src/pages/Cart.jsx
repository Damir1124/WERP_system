...
export default function Cart({ clientData, lang = 'ru' }) {
  ...
  // Раскрывающийся блок «Изменить адрес» (по клику на резюме)
  ...
  const handleCheckout = async (e) => {
    ...
    if (resolved.isEmpty) {
      ...
      document.getElementById('delivery-address-anchor-footer')?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    ...
    <div className="pb-44">
      ...
      <div className="p-4 pb-0">
        ...
        <div className="mt-5 space-y-4 bg-white rounded-2xl shadow-soft border border-gray-100 p-4">
          ...
          {/* Изменить адрес — раскрывающийся блок (по клику на резюме) */}
          {showAddressEditor && (
            <div id="delivery-address-editor" className={`space-y-4 border-t border-gray-100 pt-4 rounded-xl ${editorHighlight ? 'address-editor-highlight' : ''}`}>
        ...
        <div className="mb-3">
          <DeliveryAddressSummary
            ...
            onEdit={() => {
              ...
              setTimeout(() => document.getElementById('delivery-address-editor')?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50)
              setTimeout(() => setEditorHighlight(false), 1500)
            }}
            highlightError={addrError}
            compact
            lang={lang}
            anchorId="delivery-address-anchor-footer"
          />
        </div>
